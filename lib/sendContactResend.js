function resolveFromAddress(env) {
  const raw = String(env.CONTACT_FROM || "").trim();
  const match = raw.match(/<([^>]+)>/);
  const email = (match ? match[1] : raw).trim() || "noreply@jessymathew.me";
  // Always label Portfolio so it never reuses an old "JM International" From name.
